<template>
	<view>
		<u-query>
			<u-text size="small" v-model="query.label" label="名称" prop="label"  span="4"></u-text>
			<template v-slot:btn>
	...
	export default {
      ...
      const pageChange=(val)=>{
        ...
        search()
			...
			//新增
			const add=()=>{
				uni.$router.push({label:"新增",path:"/system/sysGroup/edit"});
			}
			//编辑
			const edit=(val)=>{
        ...
        if(objs.value.length==1){
          uni.$router.push({label:"编辑",path:"/system/sysGroup/edit",param:objs.value[0]})
        }else if(val.item!=undefined){
          uni.$router.push({label:"查看",path:"/system/sysGroup/edit",param:val.item})
        }else {
        ...
        if(val.item!=undefined){
          uni.$router.push({label:"查看",path:"/system/sysGroup/edit",param:val.item,read:true})
        } else {
          uni.$msg({message:"请选择数据",type:"warn"})
        }
			}
          ...
          uni.$msg({lx:"confirm",message:"确认删除么?",type:"warn",confirm:(e)=>{
              if(e){
                uni.request({
                  service:"system",
                  method:"GET",
                  url:"/sysGroup/del?ids="+(val.item!=undefined?val.item.id:ids.value.join(",")),
                  success:(res)=>{
                    if(res.code==200){
