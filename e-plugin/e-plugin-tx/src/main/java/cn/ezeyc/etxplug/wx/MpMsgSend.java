...
//         Arrays.sort(arr);
     ...
     * @return
     */
    public  static String getToken(wxMp wxMp) throws IOException {
        //1.获取token
        String s = HttpUtils.http(" https://api.weixin.qq.com/cgi-bin/token",
                "appid=" + wxMp.getAppId() + "&secret=" + wxMp.getAppSecret() + "&grant_type=client_credential").get();
    ...
    public  static boolean send( wxSendMsg wxSendMsg) throws IOException {
        ...
        String result =    HttpUtils.http("https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token="+ wxSendMsg.getAccessToken()).data(param).post();
